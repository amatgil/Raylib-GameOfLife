...
async fn main() {
    ...
    loop {
        ...
        draw_controls(text_color);

        println!("{}", time_between_ticks);

        next_frame().await
    }
}

fn draw_controls(text_color: Color) {
    draw_rectangle(0.0, 0.0,
                    320.0, 40.0*3.0,
                    Color::from_rgba(0, 0, 0, 200));
    draw_text("Q: Increase Speed", 10.0, 30.0 + 0.0*40.0, 40.0, text_color);
    draw_text("W: Decrease Speed", 10.0, 30.0 + 1.0*40.0, 40.0, text_color);
